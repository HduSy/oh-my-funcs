...
export const isSmallScreen = () => {
  return document.body.clientWidth / window.document.documentElement.clientHeight > 9 / 18.5
}
const mobileRe = /Android|webOS|iPhone|iPod|iOS|BlackBerry|Windows Phone|Tablet|Mobile/i
...
export const isUat = window.location.href.indexOf("uat-") >= 0 || getQueryVariable('uat') === '1'
export const isPre = window.location.href.indexOf("pre-") >= 0 || getQueryVariable('pre') === '1'
export const isAndroidPad = ua.includes('mobi_app/android_hd')
export const detectPad = !isPC && !isIpad && !isAndroidPad && document.documentElement.clientWidth > 500 // 根据设备经验，认定逻辑宽度 > 500 为平板设备（可能会对大屏手机有误伤）
